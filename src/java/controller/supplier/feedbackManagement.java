...
import service.impli.supplier.FeedbackServiceSupplier;
...
public class feedbackManagement extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet feedbackManagement</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet feedbackManagement at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        IFeedbackServiceSupplier feedbackService = new FeedbackServiceSupplier();
        String roomNo = request.getParameter("roomNo");
        long supplierId = Long.valueOf((String.valueOf(request.getSession().getAttribute("supplierId"))));
        List<FeedbackModel> list = feedbackService.getAllFeedback(Long.valueOf(roomNo), supplierId);
