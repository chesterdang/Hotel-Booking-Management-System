 ...
 * @author danhv
 */
public class displayPendingContract extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet displayPendingContract</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet displayPendingContract at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Contract> listCC = cc.displayAllPending(supplierId);
        request.getSession().setAttribute("listCC", listCC);
        if (listCC != null && (listCC.size()>0)) {
            Contract contract = listCC.get(0);
